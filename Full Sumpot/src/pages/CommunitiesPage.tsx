...
import { mockCommunitiesApi } from '../services/mockApi';
...
export default function CommunitiesPage() {
  ...
  const joinMut = useMutation({
    mutationFn: (id: string) => mockCommunitiesApi.join(id, userId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['communities', userId] });
  ...
  const createMut = useMutation({
    mutationFn: (data: FormData) => mockCommunitiesApi.create(data, userId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['communities', userId] });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((c) => (
            <div key={c.id} className="card hover:border-border/80 transition-colors flex flex-col">
              ...
              <div className="flex items-center gap-2 mt-4">
                <Link to={`/communities/${c.id}`} className="btn-secondary flex-1 text-center text-sm py-1.5">View</Link>
                {c.isMember ? (
                  <span className="px-3 py-1.5 rounded text-xs text-green-400 border border-green-900 bg-green-950/30">✓ Joined</span>
                ) : (
                  <button onClick={() => joinMut.mutate(c.id)} disabled={joinMut.isPending} className="btn-primary flex-1 text-sm py-1.5">Join</button>
                )}
              </div>
            </div>
          ))}
