...
import { mockCommunitiesApi, mockPointsApi } from '../services/mockApi';
...
export default function DashboardPage() {
  ...
  const { data: communities } = useQuery({
    ...
    queryFn: () => mockCommunitiesApi.list(userId).then((r) => r.data),
    enabled: !!userId,
  });

  const { data: points } = useQuery({
    queryKey: ['my-points', userId],
    queryFn: () => mockPointsApi.myPoints(userId).then((r) => r.data),
    enabled: !!userId,
  });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Available Points" value={points?.availablePoints ?? 0} icon={Star} color="bg-yellow-500/10 text-yellow-400" />
        <StatCard label="Points Earned Today" value={points?.pointsEarnedToday ?? 0} icon={Zap} color="bg-primary/10 text-primary" />
        <StatCard label="Views Given Today" value={points?.viewsGivenToday ?? 0} icon={Play} color="bg-green-500/10 text-green-400" />
      ...
      {joined.length > 0 && (
        <section>
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {joined.map((c) => (
              <Link key={c.id} to={`/communities/${c.id}`} className="card hover:border-primary/40 transition-colors group">
