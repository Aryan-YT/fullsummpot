...
import { mockCommunitiesApi, mockLinksApi } from '../services/mockApi';
...
function LinkCard({ link, onClick }: { link: LinkType; onClick: (id: string, url: string) => void }) {
  const videoId = link.youtubeUrl.match(/(?:v=|youtu\.be\/)([^&?]+)/)?.[1];
  ...
  return (
    <div className="card hover:border-border/80 transition-colors group">
      <div className="flex gap-4">
        <button onClick={() => onClick(link.id, link.youtubeUrl)} className="relative shrink-0 w-28 h-16 rounded overflow-hidden bg-surfaceHover">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-textSecondary mt-0.5">{formatDistanceToNow(new Date(link.createdAt), { addSuffix: true })}</p>
          <button
            onClick={() => onClick(link.id, link.youtubeUrl)}
          ...
          >
            ...
            {link.clickCount} {link.isClickedByMe ? 'viewed ✓' : 'clicks'}
          </button>
        </div>
      </div>
    </div>
...
export default function CommunityDetailPage() {
  ...
  const { data: community, isLoading: loadingCommunity } = useQuery({
    queryKey: ['community', id, userId],
    queryFn: () => mockCommunitiesApi.get(id!, userId).then((r) => r.data),
    enabled: !!id,
  });

  const { data: links, isLoading: loadingLinks } = useQuery({
    queryKey: ['links', id, userId],
    queryFn: () => mockLinksApi.list(id!, userId).then((r) => r.data),
    enabled: !!id,
  });

  const joinMut = useMutation({
    mutationFn: () => mockCommunitiesApi.join(id!, userId),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['community', id, userId] }); qc.invalidateQueries({ queryKey: ['communities', userId] }); toast.success('Joined!'); },
  });

  const leaveMut = useMutation({
    mutationFn: () => mockCommunitiesApi.leave(id!, userId),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['community', id, userId] }); qc.invalidateQueries({ queryKey: ['communities', userId] }); toast.success('Left community'); },
  });

  const clickMut = useMutation({
    mutationFn: ({ linkId, url }: { linkId: string; url: string }) =>
      mockLinksApi.click(linkId, userId).then(() => url),
    onSuccess: (url) => {
      qc.invalidateQueries({ queryKey: ['links', id, userId] });
      qc.invalidateQueries({ queryKey: ['my-points', userId] });
  ...
  const submitMut = useMutation({
    mutationFn: (data: LinkFormData) => mockLinksApi.submit(id!, data, user!),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['links', id, userId] }); toast.success('Link submitted!'); setShowSubmit(false); reset(); },
      ...
      <div>
        ...
        {loadingLinks ? (
          ...
          <div className="text-center py-12 text-textSecondary border border-dashed border-border rounded-xl">
            ...
            <p>No links yet. {community.isMember ? 'Be the first to submit!' : 'Join to submit links.'}</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 gap-4">
            {links.map((link) => (
              <LinkCard key={link.id} link={link} onClick={(linkId, url) => clickMut.mutate({ linkId, url })} />
            ))}
          </div>
        )}
      </div>
