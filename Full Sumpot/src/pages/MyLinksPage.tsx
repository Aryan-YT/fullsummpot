...
import { mockLinksApi } from '../services/mockApi';
...
export default function MyLinksPage() {
  ...
  const { data: links, isLoading } = useQuery({
    ...
    queryFn: () => mockLinksApi.myLinks(userId).then((r) => r.data),
    enabled: !!userId,
  });
  ...
  const clickMut = useMutation({
    ...
    onSuccess: (url) => {
      qc.invalidateQueries({ queryKey: ['my-links', userId] });
      ...
      {!links?.length ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {links.map((link) => {
            ...
            return (
              <div key={link.id} className="card group hover:border-border/80 transition-colors flex flex-col">
                ...
                <p className="text-xs text-textSecondary mt-1">{formatDistanceToNow(new Date(link.createdAt), { addSuffix: true })}</p>
                <div className="flex items-center justify-between mt-3">
                  <span className="flex items-center gap-1.5 text-xs text-textSecondary">
                    <MousePointerClick size={12} /> {link.clickCount} clicks
                  </span>
                  <button
                    onClick={() => clickMut.mutate({ linkId: link.id, url: link.youtubeUrl })}
