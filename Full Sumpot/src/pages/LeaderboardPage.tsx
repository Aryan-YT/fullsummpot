...
import { mockLeaderboardApi } from '../services/mockApi';
...
export default function LeaderboardPage() {
  const { data: leaderboard, isLoading } = useQuery({
    ...
    queryFn: () => mockLeaderboardApi.list().then((r) => r.data),
  });
  ...
  const topThree = leaderboard ? [leaderboard[1], leaderboard[0], leaderboard[2]].filter(Boolean) : [];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-textSecondary text-sm mt-1">Top creators by points this month</p>
      </div>
      ...
      {topThree.length > 0 && (
        <div className="grid grid-cols-3 gap-2 md:gap-4 max-w-xl mx-auto pt-6">
          {topThree.map((user, i) => {
            ...
            return (
              <div key={user.username} className={`flex flex-col items-center ${isWinner ? '-mt-6' : 'mt-0'}`}>
                <div className={`w-12 h-12 md:w-16 md:h-16 rounded-full bg-surfaceHover border-2 flex items-center justify-center mb-2 shadow-xl ${isWinner ? 'border-yellow-400 scale-110' : 'border-border'}`}>
                  <span className="text-lg md:text-xl font-bold text-textPrimary">{user.username[0]}</span>
                </div>
                <p className="text-[10px] md:text-xs font-bold text-textPrimary truncate w-full text-center">{user.username}</p>
                <p className="text-[10px] md:text-xs text-textSecondary">{user.points.toLocaleString()} pts</p>
                <div className={`w-full mt-3 rounded-t-xl bg-surfaceHover border-t border-x border-border flex flex-col items-center justify-center gap-1 ${isWinner ? 'h-32 bg-gradient-to-b from-yellow-400/10 to-surfaceHover' : 'h-24'}`}>
                  <Medal size={24} className={medalColors[user.rank - 1]} />
      ...
      <div className="card overflow-hidden p-0 border-border/50">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {leaderboard?.map((row) => (
                <tr key={row.rank} className="border-b border-border/30 hover:bg-surfaceHover/50 transition-colors group">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-xs font-bold text-primary group-hover:scale-110 transition-transform">
                        {row.username[0]}
                      </div>
                      <span className="font-semibold text-textPrimary">{row.username}</span>
                    </div>
                  </td>
                  <td className="py-4 px-4 hidden sm:table-cell">
                    <span className="px-2 py-0.5 rounded-full bg-surfaceHover text-textSecondary text-[10px] font-medium border border-border/50 uppercase tracking-wider">{row.niche}</span>
                  </td>
