...
export default function NotificationsPage() {
  ...
  const { data: notifications, isLoading } = useQuery({
    ...
    queryFn: () => mockNotificationsApi.list(userId).then((r) => r.data),
    enabled: !!userId,
  });

  const markAllMut = useMutation({
    mutationFn: () => mockNotificationsApi.markAllRead(userId),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['notifications', userId] }); toast.success('All marked as read'); },
  });
          ...
          {notifications.map((n) => (
            <div key={n.id} className={`card transition-colors ${!n.isRead ? 'border-primary/20 bg-primary/5' : ''}`}>
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="text-xs text-textSecondary shrink-0">{formatDistanceToNow(new Date(n.createdAt), { addSuffix: true })}</span>
              </div>
            </div>
          ))}
        </div>
